...
const TABS = {
  users: { title: 'users', header: ['email', 'name', 'role', 'hash', 'active', 'token', 'createdAt'] },
  reminders: { title: 'reminders', header: ['email', 'taskRow', 'methods', 'offsets', 'updatedAt'] },
...
async function ensureTabs() {
  ...
  const meta = await api().spreadsheets.get({ spreadsheetId: DATA_SHEET_ID, fields: 'sheets.properties.title' });
  ...
  if (requests.length) {
    await api().spreadsheets.batchUpdate({ spreadsheetId: DATA_SHEET_ID, requestBody: { requests } });
  }
  // كتابة صف العناوين لكل تبويب فارغ
  for (const t of Object.values(TABS)) {
    const r = await api().spreadsheets.values.get({ spreadsheetId: DATA_SHEET_ID, range: `${t.title}!A1:Z1` });
...
async function readRows(tab) {
  await ensureTabs();
  const r = await api().spreadsheets.values.get({ spreadsheetId: DATA_SHEET_ID, range: `${tab}!A2:Z10000` });
...
async function addUser({ email, name, role, hash }) {
  ...
  await api().spreadsheets.values.append({
    ...
    requestBody: { values: [[email, name || email, role || 'viewer', hash, 'TRUE', token, new Date().toISOString()]] },
  });
  invalidateUsers();
...
async function updateUser(email, patch) {
  ...
  const cur = (await api().spreadsheets.values.get({ spreadsheetId: DATA_SHEET_ID, range: `users!A${row}:G${row}` })).data.values?.[0] || [];
  while (cur.length < 7) cur.push('');
  const map = { name: 1, role: 2, hash: 3, active: 4 };
  for (const [k, v] of Object.entries(patch)) if (k in map) cur[map[k]] = typeof v === 'boolean' ? (v ? 'TRUE' : 'FALSE') : v;
  await api().spreadsheets.values.update({ spreadsheetId: DATA_SHEET_ID, range: `users!A${row}:G${row}`, valueInputOption: 'RAW', requestBody: { values: [cur] } });
  invalidateUsers();
  return true;
}
...
async function getReminders(email) {
  ...
  rows.forEach((r) => {
    if ((r[0] || '').toLowerCase() === email.toLowerCase()) {
      out[r[1]] = { methods: (r[2] || '').split(',').filter(Boolean), offsets: (r[3] || '').split(',').filter(Boolean) };
    }
  });
  return out;
}
...
async function getAllReminders() {
  if (!enabled) return [];
  const rows = await readRows('reminders');
  return rows.map((r) => ({ email: r[0], taskRow: String(r[1]), methods: (r[2] || '').split(',').filter(Boolean), offsets: (r[3] || '').split(',').filter(Boolean) }))
    .filter((x) => x.email && x.taskRow);
}

async function setReminder(email, taskRow, methods, offsets) {
  if (!enabled) throw new Error('STORE_DISABLED');
  const rows = await readRows('reminders');
  const idx = rows.findIndex((r) => (r[0] || '').toLowerCase() === email.toLowerCase() && String(r[1]) === String(taskRow));
  const value = [email, String(taskRow), methods.join(','), offsets.join(','), new Date().toISOString()];
  if (idx === -1) {
    await api().spreadsheets.values.append({ spreadsheetId: DATA_SHEET_ID, range: 'reminders!A2', valueInputOption: 'RAW', insertDataOption: 'INSERT_ROWS', requestBody: { values: [value] } });
  } else {
    const row = idx + 2;
    await api().spreadsheets.values.update({ spreadsheetId: DATA_SHEET_ID, range: `reminders!A${row}:E${row}`, valueInputOption: 'RAW', requestBody: { values: [value] } });
  }
  return true;
}
...
async function savePush(email, sub) {
  ...
  if (idx === -1) await api().spreadsheets.values.append({ spreadsheetId: DATA_SHEET_ID, range: 'push!A2', valueInputOption: 'RAW', insertDataOption: 'INSERT_ROWS', requestBody: { values: [value] } });
  else { const row = idx + 2; await api().spreadsheets.values.update({ spreadsheetId: DATA_SHEET_ID, range: `push!A${row}:C${row}`, valueInputOption: 'RAW', requestBody: { values: [value] } }); }
}

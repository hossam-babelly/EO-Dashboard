...
async function updateUser(email, patch) {
  ...
  const cur = (await api().spreadsheets.values.get({ spreadsheetId: DATA_SHEET_ID, range: `users!A${row}:I${row}` })).data.values?.[0] || [];
  while (cur.length < 9) cur.push('');
  const map = { name: 1, role: 2, hash: 3, active: 4, firstName: 7, lastName: 8 };
  ...
  await api().spreadsheets.values.update({ spreadsheetId: DATA_SHEET_ID, range: `users!A${row}:I${row}`, valueInputOption: 'RAW', requestBody: { values: [cur] } });
  invalidateUsers();
  return true;
}

...
const TABS = {
  users: { title: 'users', header: ['email', 'name', 'role', 'hash', 'active', 'token', 'createdAt', 'firstName', 'lastName', 'profiles', 'phone', 'telegramChatId', 'theme', 'digestTime'] },
  ...
  profiles: { title: 'profiles', header: ['tab', 'label', 'digestTo'] }, // digestTo = بريد مستلِم اللوحة الشاملة لهذا البروفايل
  sent: { title: 'sent', header: ['key', 'sentAt'] },
  settings: { title: 'settings', header: ['key', 'value'] }, // إعدادات عامة (مثل توقيت إرسال اللوحات اليومية)
};
...
function rowToUser(r) {
  ...
  return {
    ...
    theme: (function () { try { return r[12] ? JSON.parse(r[12]) : null; } catch (_) { return null; } })(),
    digestTime: (r[13] || '').trim(),
  };
}
...
async function updateUser(email, patch) {
  ...
  const cur = (await withRetry(() => api().spreadsheets.values.get({ spreadsheetId: DATA_SHEET_ID, range: `users!A${row}:N${row}` }))).data.values?.[0] || [];
  while (cur.length < 14) cur.push('');
  const map = { email: 0, name: 1, role: 2, hash: 3, active: 4, firstName: 7, lastName: 8, profiles: 9, phone: 10, telegramChatId: 11, theme: 12, digestTime: 13 };
  ...
  await withRetry(() => api().spreadsheets.values.update({ spreadsheetId: DATA_SHEET_ID, range: `users!A${row}:N${row}`, valueInputOption: 'RAW', requestBody: { values: [cur] } }));
  invalidateUsers();
  return true;
}

// حذف مستخدم نهائياً (حذف صفّه من تبويب users)
async function getDataSheetId(title) {
  const meta = await withRetry(() => api().spreadsheets.get({ spreadsheetId: DATA_SHEET_ID, fields: 'sheets.properties(sheetId,title)' }));
...
async function deleteUser(email) {
  if (!enabled) throw new Error('STORE_DISABLED');
  const row = await findUserRowIndex(email);
  if (row === -1) throw new Error('المستخدم غير موجود');
  const sheetId = await getDataSheetId('users');
  if (sheetId == null) throw new Error('تبويب users غير موجود');
  await withRetry(() => api().spreadsheets.batchUpdate({ spreadsheetId: DATA_SHEET_ID, requestBody: { requests: [{ deleteDimension: { range: { sheetId, dimension: 'ROWS', startIndex: row - 1, endIndex: row } } }] } }));
  invalidateUsers();
  return true;
}
...
async function getProfiles() {
  ...
  const list = rows.map((r) => ({ tab: (r[0] || '').trim(), label: (r[1] || r[0] || '').trim(), digestTo: (r[2] || '').trim() })).filter((p) => p.tab);
  // نضمن وجود البروفايل الأول دائماً في الأعلى
  if (!list.some((p) => p.tab === DEFAULT_PROFILE.tab)) list.unshift({ ...DEFAULT_PROFILE, digestTo: '' });
  return list;
}
...
async function addProfile(tab, label) {
  ...
  return { tab: name, label: String(label || name).trim(), digestTo: '' };
}

// تحديد بريد مستلِم اللوحة الشاملة لبروفايل (يُنشئ صفّ البروفايل إن لم يكن موجوداً — كحال البروفايل الأول الافتراضي)
async function setProfileDigest(tab, email) {
  if (!enabled) throw new Error('STORE_DISABLED');
  const name = String(tab || '').trim();
  if (!name) throw new Error('البروفايل مطلوب');
  const rows = await readRows('profiles');
  const idx = rows.findIndex((r) => (r[0] || '').trim() === name);
  if (idx === -1) {
    const label = name === DEFAULT_PROFILE.tab ? DEFAULT_PROFILE.label : name;
    await withRetry(() => api().spreadsheets.values.append({ spreadsheetId: DATA_SHEET_ID, range: 'profiles!A2', valueInputOption: 'RAW', insertDataOption: 'INSERT_ROWS', requestBody: { values: [[name, label, String(email || '').trim()]] } }));
  } else {
    const row = idx + 2;
    const cur = (await withRetry(() => api().spreadsheets.values.get({ spreadsheetId: DATA_SHEET_ID, range: `profiles!A${row}:C${row}` }))).data.values?.[0] || [];
    while (cur.length < 3) cur.push('');
    cur[2] = String(email || '').trim();
    await withRetry(() => api().spreadsheets.values.update({ spreadsheetId: DATA_SHEET_ID, range: `profiles!A${row}:C${row}`, valueInputOption: 'RAW', requestBody: { values: [cur] } }));
  }
  return true;
}
...
async function getSetting(key, def) {
  ...
  const r = rows.find((x) => (x[0] || '').trim() === key);
  return r ? (r[1] != null ? String(r[1]) : def) : def;
}
async function setSetting(key, value) {
  if (!enabled) throw new Error('STORE_DISABLED');
  const rows = await readRows('settings');
  const idx = rows.findIndex((x) => (x[0] || '').trim() === key);
  const val = [String(key), String(value == null ? '' : value)];
  if (idx === -1) await withRetry(() => api().spreadsheets.values.append({ spreadsheetId: DATA_SHEET_ID, range: 'settings!A2', valueInputOption: 'RAW', insertDataOption: 'INSERT_ROWS', requestBody: { values: [val] } }));
  else { const row = idx + 2; await withRetry(() => api().spreadsheets.values.update({ spreadsheetId: DATA_SHEET_ID, range: `settings!A${row}:B${row}`, valueInputOption: 'RAW', requestBody: { values: [val] } })); }
  return true;
}
...
async function setUserTheme(email, themeJson) {
  return updateUser(String(email).toLowerCase(), { theme: themeJson || '' });
}
// بثّ تصميم واحد لكل المستخدمين: كتابة العمود M (theme) دفعةً واحدة
async function broadcastTheme(themeJson) {
  if (!enabled) throw new Error('STORE_DISABLED');
  const rows = await readRows('users');
  const n = rows.length;
  if (!n) return 0;
  const val = String(themeJson == null ? '' : themeJson);
  const col = rows.map(() => [val]);
  await withRetry(() => api().spreadsheets.values.update({ spreadsheetId: DATA_SHEET_ID, range: `users!M2:M${n + 1}`, valueInputOption: 'RAW', requestBody: { values: col } }));
  invalidateUsers();
  return n;
}

// ===== توقيت اللوحة اليومية لكل مستخدم + تعميم موحّد =====
async function setUserDigestTime(email, time) {
  return updateUser(String(email).toLowerCase(), { digestTime: String(time == null ? '' : time).trim() });
}
// تعميم توقيت واحد على كل المستخدمين: كتابة العمود N (digestTime) دفعةً واحدة
async function broadcastDigestTime(time) {
  if (!enabled) throw new Error('STORE_DISABLED');
  const rows = await readRows('users');
  const n = rows.length;
  if (!n) return 0;
  const val = String(time == null ? '' : time).trim();
  const col = rows.map(() => [val]);
  await withRetry(() => api().spreadsheets.values.update({ spreadsheetId: DATA_SHEET_ID, range: `users!N2:N${n + 1}`, valueInputOption: 'RAW', requestBody: { values: col } }));
  invalidateUsers();
  return n;
}
...
module.exports = {
  ...
  getUsersFull, addUser, updateUser, deleteUser, getUserByToken, invalidateUsers,
  setUserTheme, broadcastTheme, setUserDigestTime, broadcastDigestTime,
  getReminders, getAllReminders, setReminder,
  savePush, getPushByEmail,
  getProfiles, addProfile, setProfileDigest,

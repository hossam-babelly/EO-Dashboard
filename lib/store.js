...
const TABS = {
  users: { title: 'users', header: ['email', 'name', 'role', 'hash', 'active', 'token', 'createdAt', 'firstName', 'lastName', 'profiles', 'phone', 'telegramChatId', 'theme'] },
...
async function updateUser(email, patch) {
  ...
  const cur = (await withRetry(() => api().spreadsheets.values.get({ spreadsheetId: DATA_SHEET_ID, range: `users!A${row}:M${row}` }))).data.values?.[0] || [];
  while (cur.length < 13) cur.push('');
  const map = { email: 0, name: 1, role: 2, hash: 3, active: 4, firstName: 7, lastName: 8, profiles: 9, phone: 10, telegramChatId: 11, theme: 12 };
  ...
  await withRetry(() => api().spreadsheets.values.update({ spreadsheetId: DATA_SHEET_ID, range: `users!A${row}:M${row}`, valueInputOption: 'RAW', requestBody: { values: [cur] } }));
  invalidateUsers();
  return true;
}

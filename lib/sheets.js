...
async function normalizeLogs(rows, h, cols) {
  ...
  for (let i = h + 1; i < rows.length; i++) {
    ...
    if (fb.length > lb.length) {
      const padded = lb.concat(new Array(fb.length - lb.length).fill('----------'));
      data.push({ range: range(`${numToLetter(cols.log)}${i + 1}`), values: [[padded.join('\n\n')]] });
    }
  }
  if (data.length) {
    try { await sheetsApi().spreadsheets.values.batchUpdate({ spreadsheetId: SHEET_ID, requestBody: { valueInputOption: 'USER_ENTERED', data } }); }

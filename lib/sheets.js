...
// تحويل مصفوفة اجتماعات [{title, scheduled}] إلى عمودَي الشيت المتوازيين (كتل مفصولة بسطر فارغ)
function meetingsToCols(meetings) {
  const valid = (Array.isArray(meetings) ? meetings : [])
    .map((m) => ({
      ...
      scheduled: !!(m && m.scheduled),
    }))
    .filter((m) => m.title);
  return {
    meeting: valid.map((m) => m.title).join('\n\n'),
    scheduled: valid.map((m) => (m.scheduled ? 'تم' : 'لا')).join('\n\n'),
  };
}
...
async function colorCells(items) {
...
function buildTask(r, rowNumber, cols) {
  ...
  const meetings = mTitles.map((title, i) => ({ title, scheduled: SCHED_TRUTHY.test((mScheds[i] || '').trim()) }));
...
async function normalizeLogs(rows, h, cols) {
  ...
  for (let i = h + 1; i < rows.length; i++) {
    ...
    if (fb.length !== lb.length) {
      ...
      data.push({ range: range(`${numToLetter(cols.log)}${i + 1}`), values: [[newLog]] });
  ...
  if (data.length) {
    try {
      await sheetsApi().spreadsheets.values.batchUpdate({ spreadsheetId: SHEET_ID, requestBody: { valueInputOption: 'USER_ENTERED', data } });
      await colorCells(colorItems);
...
async function updateTask(rowNumber, patch) {
  ...
  for (const [field, value] of Object.entries(expandMeetings(patch))) {

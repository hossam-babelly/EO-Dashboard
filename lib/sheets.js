...
const HEADER_ALIASES = {
  ...
  // موعد كل مخرَج (كتل متوازية لكتل «المخرج المطلوب»؛ «-» = بلا موعد). يُضاف عادةً يسار «المخرج المطلوب».
  dvdates: ['مواعيد المخرجات', 'مواعيد المخرَجات', 'تواريخ المخرجات', 'موعد المخرجات'],
  // تاريخ إنجاز كل مخرَج (كتل متوازية؛ «-» = غير منجَز). يُملأ تلقائياً لحظة التأشير — لتجميد عدّاد الأيام.
  dvdone: ['مواعيد إنجاز المخرجات', 'تاريخ إنجاز المخرجات', 'إنجاز المخرجات'],
};

// الأعمدة التي يضمن التطبيق وجودها (أعمدة الاجتماعات يديرها المالك يدوياً كنص)
const MANAGED_COLUMNS = ['الحالة', 'تاريخ الإنجاز', 'مكلّف المخرجات', 'مواعيد إنجاز المخرجات'];
...
// قيمة كتلة متوازية «حقيقية» أم placeholder فارغ («-»/«—»/«----------»)
function realBlockVal(v) {
  const s = String(v == null ? '' : v).trim();
  return (s && s !== '-' && s !== '—' && s !== '----------') ? s : '';
}
// فرق أيام بين تاريخَي ISO (a − b)
function isoDayDiff(a, b) {
  const [ay, am, ad] = a.split('-').map(Number);
  const [by, bm, bd] = b.split('-').map(Number);
  return Math.round((Date.UTC(ay, am - 1, ad) - Date.UTC(by, bm - 1, bd)) / 86400000);
}
// بناء مصفوفة المخرجات ككائنات: نصّ + إنجاز + مكلّف + موعد + تاريخ الإنجاز.
// الكتل متوازية بين «المخرج المطلوب» و«مكلّف المخرجات» و«مواعيد المخرجات» و«مواعيد إنجاز المخرجات».
// عدّاد الأيام (diffDays): للمخرَج غير المنجَز = الموعد − اليوم (حيّ)؛ وللمنجَز = الموعد − تاريخ الإنجاز (مُجمَّد).
function buildDeliverables(deliverableRaw, assigneesRaw, datesRaw, doneRaw) {
  const dB = fuBlocks(deliverableRaw);
  ...
  return dB.map((b, i) => {
    ...
    const dateRaw = realBlockVal(tB[i]);
    // toIsoDate يدعم ISO وD/M/Y بشكل صحيح (لا parseDeadline المنحازة لـ D/M/Y)
    const dateIso = dateRaw ? toIsoDate(dateRaw) : '';
    const doneDateRaw = realBlockVal(nB[i]);
    const doneDateIso = doneDateRaw ? toIsoDate(doneDateRaw) : '';
    let diffDays = null, isOverdue = false, isToday = false, isSoon3 = false;
    if (dateIso) {
      if (done) {
        // مُجمَّد على لحظة الإنجاز (الموعد − تاريخ الإنجاز)؛ منجَز قديم بلا تاريخ إنجاز ⇒ لا عدّاد
        diffDays = doneDateIso ? isoDayDiff(dateIso, doneDateIso) : null;
      } else {
        const flags = classify({ iso: dateIso, recurrence: null }, false);
        diffDays = flags.diffDays; isOverdue = flags.isOverdue; isToday = flags.isToday; isSoon3 = flags.isSoon3;
      }
    }
    return {
      idx: i,
      ...
      text,
      assignee: realBlockVal(aB[i]),
      dateRaw,
      dateIso: dateIso || null,
      doneDateRaw,
      doneDateIso: doneDateIso || null,
      diffDays,
      isOverdue,
      isToday,
      isSoon3,
...
function buildTask(r, rowNumber, cols) {
  ...
  return {
    ...
    // مواعيد المخرجات + مواعيد إنجازها (نصّ خام) + المخرجات ككائنات محلَّلة (نصّ/إنجاز/مكلّف/موعد مُصنَّف/تاريخ إنجاز)
    dvdates: get('dvdates'),
    dvdone: get('dvdone'),
    deliverables: buildDeliverables(get('deliverable'), get('dvowners'), get('dvdates'), get('dvdone')),
    status,
    isDone,
    isMeeting,
    meetings,
...
async function updateTask(tab, rowNumber, patch) {
  ...
  // تلوين الأعمدة ذات الكتل المتوازية فور الكتابة (يبقى التلوين دائماً حتى عند الكتابة عبر الرابط/API،
  // إذ لا يُطلَق onEdit في السكربت إلا عند التعديل اليدوي). يشمل: المتابعة/السجل + المخرج المطلوب/مواعيد المخرجات.
  await colorCells(tab, [
    { row, col: cols.followup, text: current[cols.followup] },
    { row, col: cols.log, text: current[cols.log] },
    { row, col: cols.deliverable, text: current[cols.deliverable] },
    { row, col: cols.dvdates, text: current[cols.dvdates] },
  ]);
  return buildTask(current, row, cols);
}

/** إضافة مهمة جديدة في أول صفّ خالٍ فعلياً بعد العناوين. */
async function addTask(tab, task) {
  if (!canWrite) throw new Error('WRITE_DISABLED');
  // نقرأ القيم الفعلية لتحديد صفّ العناوين وأول صفّ خالٍ — لا نعتمد على values.append لأنه يتأثّر بنطاق
  // القوائم المنسدلة/التنسيق على الصفوف الفارغة (كما في تبويب منسوخ) فيضع المهمة في مكان بعيد للأسفل.
  const res = await withRetry(() => sheetsApi().spreadsheets.values.get({
    spreadsheetId: SHEET_ID, range: range(tab, `A1:${WIDE_COL}2000`),
  }));
  const rows = res.data.values || [];
  let h = findHeaderIdx(rows); if (h === -1) h = 1;
  const cols = mapColumns(rows[h] || []);
  const maxIdx = Math.max(-1, ...Object.values(cols));
  ...
  // أول صفّ بعد العناوين خالٍ من المحتوى الفعلي (نتجاوز الصفوف المملوءة المتتالية فقط)
  let targetIdx = h + 1;
  while (targetIdx < rows.length && rowHasContent(rows[targetIdx] || [], cols)) targetIdx++;
  const targetRow = targetIdx + 1; // رقم الصفّ الفعلي (1-based)
  await withRetry(() => sheetsApi().spreadsheets.values.update({
    spreadsheetId: SHEET_ID,
    range: range(tab, `A${targetRow}:${numToLetter(row.length - 1)}${targetRow}`),
    valueInputOption: 'USER_ENTERED',
    requestBody: { values: [row] },
  }));
  // تلوين كتل المهمة الجديدة فور إنشائها (كتابة API لا تُطلق onEdit في السكربت)
  await colorCells(tab, [
    { row: targetRow, col: cols.followup, text: row[cols.followup] },
    { row: targetRow, col: cols.log, text: row[cols.log] },
    { row: targetRow, col: cols.deliverable, text: row[cols.deliverable] },
    { row: targetRow, col: cols.dvdates, text: row[cols.dvdates] },
  ]);
  return true;
}
...
/** إعادة تسمية قيمة حقل بتطابق كامل عبر كل صفوف التبويب (مثل: اسم المشروع). يعيد عدد الصفوف المُحدَّثة. */
async function renameFieldValue(tab, field, from, to) {
  if (!canWrite) throw new Error('WRITE_DISABLED');
  const f = String(from == null ? '' : from).trim();
  const t = String(to == null ? '' : to).trim();
  if (!f || !t) throw new Error('BAD_ARGS');
  const res = await withRetry(() => sheetsApi().spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: range(tab, `A1:${WIDE_COL}2000`) }));
  const rows = res.data.values || [];
  let h = findHeaderIdx(rows); if (h === -1) h = 1;
  const cols = mapColumns(rows[h] || []);
  const col = cols[field];
  if (col == null) throw new Error('عمود غير موجود');
  const data = [];
  for (let i = h + 1; i < rows.length; i++) {
    const r = rows[i] || [];
    if (norm(r[col]) === norm(f)) data.push({ range: range(tab, `${numToLetter(col)}${i + 1}`), values: [[t]] });
  }
  if (data.length) await withRetry(() => sheetsApi().spreadsheets.values.batchUpdate({ spreadsheetId: SHEET_ID, requestBody: { valueInputOption: 'USER_ENTERED', data } }));
  return data.length;
}

/** إعادة تسمية اسم شخص داخل عمود «مرتبط بـ» (قائمة متعددة) عبر كل الصفوف. يعيد عدد الصفوف المُحدَّثة. */
async function renameLinkedName(tab, from, to) {
  if (!canWrite) throw new Error('WRITE_DISABLED');
  const f = String(from == null ? '' : from).trim();
  const t = String(to == null ? '' : to).trim();
  if (!f || !t) throw new Error('BAD_ARGS');
  ...
  for (let i = h + 1; i < rows.length; i++) {
    ...
    let changed = false;
    const parts = splitLinked(raw).map((p) => { if (norm(p) === norm(f)) { changed = true; return t; } return p; });
    if (changed) data.push({ range: range(tab, `${numToLetter(col)}${i + 1}`), values: [[parts.join('\n')]] });
  }
  if (data.length) await withRetry(() => sheetsApi().spreadsheets.values.batchUpdate({ spreadsheetId: SHEET_ID, requestBody: { valueInputOption: 'USER_ENTERED', data } }));
  return data.length;
}

...
async function normalizeLogs(rows, h, cols) {
  ...
  for (let i = h + 1; i < rows.length; i++) {
    ...
    if (fb.length > lb.length) {
      const paddedLog = lb.concat(new Array(fb.length - lb.length).fill('----------')).join('\n\n');
      data.push({ range: range(`${numToLetter(cols.log)}${i + 1}`), values: [[paddedLog]] });
      colorItems.push({ row: i + 1, col: cols.followup, text: r[cols.followup] });
      colorItems.push({ row: i + 1, col: cols.log, text: paddedLog });
    }
  }
  if (data.length) {
    try {

...
function findHeaderIdx(rows) {
  return rows.findIndex(
    (r) => Array.isArray(r)
      && r.some((c) => String(c).trim() === 'المشروع')
      && r.some((c) => ['المسؤول المعني', 'المخرج المطلوب'].includes(String(c).trim()))
  );
}
...
async function getTasks() {
  const res = await sheetsApi().spreadsheets.values.get({
    ...
    range: range(`A1:${LAST_COL_LETTER}1000`),
    ...keyParam(),
  });
  const rows = res.data.values || [];
  let h = findHeaderIdx(rows);
  if (h === -1) h = 1; // افتراضي: العنوان في الصف ٢
  const tasks = [];
  for (let i = h + 1; i < rows.length; i++) {
    if (rowHasContent(rows[i])) tasks.push(buildTask(rows[i], i + 1)); // i+1 = رقم الصف الفعلي
  }
  return tasks;
}

/** التأكد من وجود عمود «الحالة» في صف العناوين المكتشَف (يتطلب صلاحية كتابة). */
async function ensureStatusColumn() {
  if (!canWrite) return false;
  const res = await sheetsApi().spreadsheets.values.get({
    spreadsheetId: SHEET_ID,
    range: range(`A1:${LAST_COL_LETTER}60`),
  });
  const rows = res.data.values || [];
  ...
  await sheetsApi().spreadsheets.values.update({
    spreadsheetId: SHEET_ID,
    range: range(`${LAST_COL_LETTER}${h + 1}`),

...
// بناء مصفوفة المخرجات ككائنات: نصّ + إنجاز + مكلّف + موعد (مع تصنيفه الزمني نسبةً لليوم).
// الكتل متوازية بين «المخرج المطلوب» و«مكلّف المخرجات» و«مواعيد المخرجات».
function buildDeliverables(deliverableRaw, assigneesRaw, datesRaw) {
  ...
  return dB.map((b, i) => {
    ...
    // نستخدم toIsoDate (يدعم ISO وD/M/Y بشكل صحيح) لا parseDeadline (المنحازة لـ D/M/Y فتُسيء قراءة ISO)
    const dateIso = dateRaw ? toIsoDate(dateRaw) : '';
    const flags = dateIso ? classify({ iso: dateIso, recurrence: null }, done) : null;
    return {
      idx: i,
      done,
      text,
      ...
      diffDays: flags ? flags.diffDays : null,
      isOverdue: flags ? flags.isOverdue : false,
      isToday: flags ? flags.isToday : false,
      isSoon3: flags ? flags.isSoon3 : false,
    };
  });
}
...
function buildTask(r, rowNumber, cols) {
  ...
  return {
    ...
    // مواعيد المخرجات (نصّ خام بكتل متوازية) + المخرجات ككائنات محلَّلة (نصّ/إنجاز/مكلّف/موعد مُصنَّف)
    dvdates: get('dvdates'),
    deliverables: buildDeliverables(get('deliverable'), get('dvowners'), get('dvdates')),
    status,
    isDone,
    isMeeting,
    meetings,

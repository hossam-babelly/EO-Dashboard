  ...
  for (const [email, g] of Object.entries(byUser)) {
    ...
    if (telegram.enabled && g.telegram.length) {
      ...
      if (u && u.telegramChatId) {
        const lines = g.telegram.map((x) => `• ${x.t.project}${x.t.file ? ' — ' + x.t.file : ''} (${x.t.deadlineRaw || ''})`).join('\n');
...
module.exports = {
  ...
  dueOffsetsToday, sendDueReminders, sendScheduledReminders,
};

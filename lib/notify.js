...
/**
 * «مواعيد الإرسال القادمة» (لآلية الإيقاظ المسبق — الحزمة 34):
 * يجمع كل لحظات الإرسال المتوقّعة خلال الساعات القادمة (~26 ساعة) دون إرسال شيء:
 * ظهورات التذكيرات (الصريحة + الافتراضي الضمني للمسؤولين + الاجتماعات + المخرجات المؤرّخة)
 * + توقيتات اللوحات اليومية (توقيت كل مستخدم الفعّال + التوقيت الموحّد).
 * يعيد قائمة مرتّبة من طوابع «YYYY-MM-DD HH:MM» بتوقيت دمشق. الخادم يكتبها في
 * settings.upcoming_sends ليقرأها سكربت Apps Script مباشرةً من الشيت (دون إيقاظ الخادم)
 * فيوقظ Render قبل كل موعد بمقدار «الإيقاظ المسبق» حتى خارج فترة عمل النبضة.
 * ملاحظة: منطق الأهداف هنا مرآة لمنطق sendScheduledReminders — أي تعديل هناك يُطبَّق هنا.
 */
async function collectUpcomingSends(tasksByRow, reminders, store, opts = {}) {
  const now = opts.now ? new Date(opts.now).getTime() : Date.now();
  // أفق 50 ساعة: يغطي يوماً معطَّلاً كاملاً بين فترتي عمل (عطلة) بهامش؛ وكل استيقاظ (نبضة/إيقاظ مسبق)
  // يعيد تحديث القائمة فتتسلسل التغطية إلى ما بعد الأفق تلقائياً.
  const horizonMs = (opts.horizonHours || 50) * 3600000;
  const out = new Set();
  const add = (epoch) => { if (epoch >= now - 60000 && epoch <= now + horizonMs) out.add(damascusFull(epoch)); };
  // نطاق الظهورات: اليوم + الغد + بعد الغد (دمشقياً) — الماضي يُستبعد في add
  const allowedDates = [damascusDateStr(now), damascusDateStr(now + 86400000), damascusDateStr(now + 2 * 86400000)];

  let usersByEmail = {}; const usersByName = {};
  if (store) { try { (await store.getUsersFull() || []).forEach((u) => { usersByEmail[(u.email || '').toLowerCase()] = u; if (u.name) usersByName[String(u.name).trim()] = u; }); } catch { /* تجاهل */ } }

  // التذكير الافتراضي الضمني للمسؤولين (نفس منطق sendScheduledReminders تماماً)
  const allReminders = (reminders || []).slice();
  const explicitTaskKeys = new Set(allReminders.filter((r) => !(r.meeting !== '' && r.meeting != null)).map((r) => `${(r.email || '').toLowerCase()}|${r.taskRow}`));
  const seenDefault = new Set();
  for (const [row, t] of Object.entries(tasksByRow || {})) {
    if (!t || t.isDone || !t.deadlineIso) continue;
    for (const ownerName of (t.owners || [])) {
  ...
  for (const r of allReminders) {
    ...
    if (isMtg) {
      ...
      bases.push({ occBase: { deadlineIso: (m.datetime || '').slice(0, 10) || null }, pref: r });
    } else {
      if (t.isDone) continue;
      bases.push({ occBase: t, pref: r });
      const prefNoDates = { ...r, dates: [] };
      for (const d of (t.deliverables || [])) {
        if (d.done || !d.dateIso) continue;
        bases.push({ occBase: { deadlineIso: d.dateIso }, pref: prefNoDates });
      }
    }
    for (const b of bases) for (const o of reminderOccurrences(b.occBase, b.pref, allowedDates)) add(o.epoch);
  }

  // اللوحات اليومية: التوقيت الفعّال لكل مستخدم نشط + التوقيت الموحّد (لمستلمين بلا حساب) — اليوم والغد
  if (store) {
    try {
      const globalTime = await store.getSetting('digestTime', '');
      const validHM = (s) => /^\d{1,2}:\d{2}$/.test(String(s || ''));
      const hms = new Set();
      if (validHM(globalTime)) hms.add(globalTime);
      Object.values(usersByEmail).forEach((u) => {
        if (u.active === false) return;
        const t = validHM(u.digestTime) ? u.digestTime : globalTime;
        if (validHM(t)) hms.add(t);
      });
      for (const hm of hms) {
        const [h, m] = String(hm).split(':').map(Number);
        for (const date of allowedDates) add(damascusEpoch(date, h || 0, m || 0));
      }
    } catch { /* تجاهل */ }
  }
...
async function sendDailyBoards(profiles, tasksByProfile, store, appUrl, opts = {}) {
  ...
  const windowMin = opts.windowMin != null ? opts.windowMin : 180; // يُرسل خلال ٣ ساعات من توقيت المستخدم (تحمّلاً لتأخّر النبضة)
  const validHM = (s) => /^\d{1,2}:\d{2}$/.test(String(s || ''));
  const inWindow = (t) => { if (!validHM(t)) return false; const [h, m] = String(t).split(':').map(Number); const dm = h * 60 + m; return nowMin >= dm && nowMin <= dm + windowMin; };
  // توقيت الإرسال أصبح لكل مستخدم (users.digestTime)؛ settings.digestTime = التوقيت الموحّد/الافتراضي لمن لم يُخصَّص له توقيت.
  const globalTime = await store.getSetting('digestTime', '');
  const effTime = (u) => (u && validHM(u.digestTime)) ? u.digestTime : globalTime;

  // خرائط المستخدمين تُحمَّل مبكّراً (قبل قراءة سجل الإرسال) لحساب الاستحقاق حسب توقيت كلٍّ.
  ...
  // الوضع العادي: تخطٍّ سريع إن لم يكن أحدٌ ضمن نافذة توقيته الآن (يوفّر نداءات Google بقية اليوم). التجريبي (force) يتجاوز التوقيت ومنع التكرار.
  if (!opts.force) {
    const anyDue = inWindow(globalTime) || Object.values(usersByEmail).some((u) => inWindow(effTime(u)));
    if (!anyDue) { result.skipped = 'outside-window'; return result; }
  }
  const shouldSend = (u) => opts.force || inWindow(effTime(u)); // u قد يكون undefined لمستلِم بلا حساب → يقع على التوقيت الموحّد

  const todayStr = damascusDateStr(now);
  const canMail = emailEnabled();
  const sentSet = (!opts.force && store) ? await store.getSentKeys() : new Set();
  // منع التكرار مستقلّ لكل قناة (بريد/تيليجرام)؛ مفتاح تيليجرام يعتمد على chatId (يدمج المستلِمين المشتركين بحساب واحد).
  // التسجيل تدريجي: تُكتب المفاتيح في تبويب sent فور نجاح كل إرسال (لا في نهاية الدورة فقط) —
  // كي لا يعيد أي تداخل/انقطاع منتصف الدورة إرسال ما سبق إرساله فعلاً.
  let _pend = [];
  const mark = (k) => { _pend.push(k); sentSet.add(k); };
  const persistMarks = async () => {
    if (opts.force || !_pend.length) return;
    const batch = _pend; _pend = [];
    try { await store.markSent(batch); }
    catch (e) { _pend = batch.concat(_pend); result.errors.push('markSent: ' + e.message); }
  };
  ...
  for (const p of profiles) {
    ...
    if (compRecipients.length && (total || mtot)) {
      ...
      for (const rcpt of compRecipients) {
        ...
        if (!shouldSend(u)) continue; // ضمن نافذة توقيت هذا المستلِم فقط (أو الموحّد إن بلا حساب)
        let any = false;
        // بريد بمفتاح مستقلّ
        const kEmail = `board|comp|${p.tab}|${rcpt}|${todayStr}|email`;
        if (canMail && !sentSet.has(kEmail)) {
          try { await sendEmail({ to: rcpt, subject: `لوحة المهام والاجتماعات — ${p.label || p.tab} (${total} مهمة · ${mtot} اجتماع)`, html }); result.email++; any = true; mark(kEmail); }
        ...
        if (telegram.enabled && u && u.telegramChatId) {
          ...
          if (!sentSet.has(kTg)) {
            try { const ok = await telegram.sendMessage(u.telegramChatId, text); if (ok) { result.telegram++; any = true; mark(kTg); } }
            catch (e) { result.errors.push('comp tg ' + p.tab + ': ' + e.message); }
          }
        }
        if (any) result.boards++;
        await persistMarks(); // تسجيل فوري بعد كل مستلِم
      }
    }
    ...
    for (const ownerName of Object.keys(byOwner)) {
      ...
      if (!u || !u.email) continue;
      if (!shouldSend(u)) continue; // ضمن نافذة توقيت هذا المسؤول فقط
      ...
      if (canMail && !sentSet.has(kEmail)) {
        try { await sendEmail({ to: u.email, subject: `مهامك واجتماعاتك اليومية — ${p.label || p.tab} (${ptotal} مهمة · ${pmtot} اجتماع)`, html: buildPersonalBoardHtml(u.name, p.label || p.tab, pb, pmb, appUrl) }); result.email++; any = true; mark(kEmail); }
      ...
      if (telegram.enabled && u.telegramChatId) {
        ...
        if (!sentSet.has(kTg)) {
          try { const ok = await telegram.sendMessage(u.telegramChatId, boardTextPersonal(u.name, p.label || p.tab, pb, pmb, appUrl)); if (ok) { result.telegram++; any = true; mark(kTg); } }
          catch (e) { result.errors.push('pers tg ' + u.email + ': ' + e.message); }
        }
      }
      if (any) result.boards++;
      await persistMarks(); // تسجيل فوري بعد كل مسؤول
    }
  }
  await persistMarks(); // تسجيل أي متبقٍّ (إعادة محاولة ما فشل تسجيله أثناء الدورة)
  return result;
}
...
module.exports = {
  ...
  sendDailyBoards, DEFAULT_OWNER_REMINDER, collectUpcomingSends,
};

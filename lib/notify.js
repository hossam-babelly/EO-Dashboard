...
async function sendDailyBoards(profiles, tasksByProfile, store, appUrl, opts = {}) {
  ...
  const canMail = emailEnabled();
  const sentSet = await store.getSentKeys();
  const toMark = [];
  ...
  for (const p of profiles) {
    ...
    // اللوحة الشاملة → مستلِم البروفايل المحدّد من المدير
    if (p.digestTo && total) {
      const key = `board|comp|${p.tab}|${todayStr}`;
      if (!sentSet.has(key)) {
        const u = usersByEmail[String(p.digestTo).toLowerCase()];
        let any = false;
        if (canMail) { try { await sendEmail({ to: p.digestTo, subject: `لوحة المهام الشاملة — ${p.label || p.tab} (${total})`, html: buildComprehensiveBoardHtml(p.label || p.tab, b, appUrl) }); result.email++; any = true; } catch (e) { result.errors.push('comp email ' + p.tab + ': ' + e.message); } }
        if (telegram.enabled && u && u.telegramChatId) { try { const ok = await telegram.sendMessage(u.telegramChatId, boardTextComprehensive(p.label || p.tab, b, appUrl)); if (ok) { result.telegram++; any = true; } } catch (e) { result.errors.push('comp tg ' + p.tab + ': ' + e.message); } }
  ...
  if (toMark.length) { try { await store.markSent(toMark); } catch (e) { result.errors.push('markSent: ' + e.message); } }
  return result;
}

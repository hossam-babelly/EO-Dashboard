...
const OFFSET_LABEL = { morning: 'صباح اليوم', '1d': 'قبل يوم', '3d': 'قبل ٣ أيام', '7d': 'قبل أسبوع' };
...
const emailEnabled = () => useBrevo() || !!process.env.SMTP_HOST;
function senderEmail() { return process.env.SMTP_FROM || process.env.SENDER_EMAIL || process.env.SMTP_USER || 'no-reply@eo-dashboard'; }
function senderName() { return process.env.SENDER_NAME || 'لوحة المكتب التنفيذي'; }

/** إرسال بريد عبر Brevo API أو SMTP (واجهة موحّدة). */
...
module.exports = {
  getTransporter, sendEmail, emailEnabled, initVapid, addSubscription, removeSubscription, subscriptionCount,

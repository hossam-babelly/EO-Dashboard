...
async function call(method, body, _retried) {
  const res = await fetch(`${API}/${method}`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body),
  });
  const data = await res.json().catch(() => ({}));
  if (!data.ok) {
    // تجاوز حدّ المعدّل (429): تيليجرام يعيد retry_after بالثواني — ننتظرها ونعيد المحاولة مرّة واحدة
    const retryAfter = data && data.parameters && data.parameters.retry_after;
    if (res.status === 429 && retryAfter && !_retried) {
      await new Promise((r) => setTimeout(r, (Number(retryAfter) + 1) * 1000));
      return call(method, body, true);
    }
    throw new Error((data && data.description) || ('Telegram ' + res.status));
  }
  return data.result;
}

// رسالة فاصلة مرئية تُرسَل قبل كل رسالة (أيّاً كان نوعها) لتفصل الرسائل بصرياً على تيليجرام.
// صامتة (disable_notification) كي لا يصدر تنبيه مزدوج — الرسالة الفعلية وحدها تُصدر التنبيه.
const DIVIDER = '➖➖➖➖➖➖➖➖➖➖➖➖';
async function sendDivider(chatId) {
  try { await call('sendMessage', { chat_id: chatId, text: DIVIDER, disable_web_page_preview: true, disable_notification: true }); }
  catch (e) { /* تجاهل فشل الفاصلة — لا يجب أن يمنع الرسالة الفعلية */ }
}

// حدّ تيليجرام 4096 حرفاً للرسالة؛ نقسّم ما يتجاوزه عند حدود الأسطر (كي لا تنكسر وسوم <b>/<i>/<s>
// التي تقع كلّها ضمن سطر واحد في نصوص اللوحات). نفضّل القطع عند سطر فارغ ثم سطر عادي.
const TG_MAX = 3900; // أقلّ من 4096 احتياطاً للترميز
function splitForTelegram(text, max) {
  const s = String(text);
  if (s.length <= max) return [s];
  const parts = [];
  let rest = s;
  while (rest.length > max) {
    let cut = rest.lastIndexOf('\n\n', max);            // الأفضل: حدّ فقرة (بين مجموعات/تصنيفات)
    if (cut < max * 0.5) cut = rest.lastIndexOf('\n', max); // وإلا: أيّ سطر جديد
    if (cut < max * 0.5) cut = max;                     // وإلا (سطر طويل نادر): اقطع عند الحدّ
...
async function sendMessage(chatId, text) {
  ...
  try {
    await sendDivider(chatId); // فاصلة مرئية واحدة قبل الرسالة (حتى لو قُسِّمت لأجزاء)
    const parts = splitForTelegram(text, TG_MAX);
    for (const part of parts) {
      await call('sendMessage', { chat_id: chatId, text: part, parse_mode: 'HTML', disable_web_page_preview: true });
    }
    return true;
  }

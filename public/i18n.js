...
(function () {
  const DICT = {
    ...
    'داخل الفترة المحددة يعمل الزناد كل دقيقة: الخادم مستيقظ، المنصة سريعة، والتذكيرات لحظية. خارجها يتوقف فينام الخادم لتوفير ساعات الاستضافة — وأول فتح للرابط بعد النوم يستغرق نحو دقيقة. «إجبار الإيقاظ» يوقظ الخادم قبل كل موعد إرسال (تذكير أو لوحة يومية) بالمهلة المحددة حتى وهو نائم، فتصل الرسائل في وقتها.': 'Within the selected window the trigger runs every minute: the server stays awake, the platform is fast, and reminders are instant. Outside it, the trigger stops and the server sleeps to save hosting hours — the first visit after sleep takes about a minute. “Force wake” wakes the server the set lead time before every scheduled send (reminder or daily board) even while it sleeps, so messages arrive on time.',
    'النبضة مفعّلة (طرقات كل دقيقة داخل الفترة المحددة — إلغاء التحديد يحفظ الإعدادات)': 'Tick enabled (minute pings within the set window — unchecking keeps the settings)',
    'إجبار الإيقاظ قبل أي إرسال (تذكير/لوحة) — يعمل حتى لو كانت النبضة معطّلة': 'Force wake before any send (reminder/board) — works even while the tick is disabled',
    'الحالات: كلاهما مفعّل = فترة عمل + إيقاظ قبل كل إرسال خارجها · النبضة فقط = الفترة حصراً (ما خرج عنها لا يصل) · الإجبار فقط = الخادم نائم دائماً ويستيقظ قبل كل إرسال فقط · كلاهما معطّل = لا يستيقظ أبداً (لا تذكيرات ولا لوحات).': 'Cases: both on = working window + wake before every send outside it · tick only = the window strictly (anything outside it is not delivered) · force only = the server always sleeps and wakes only before each send · both off = never wakes (no reminders, no boards).',
    'أيام العمل:': 'Working days:',
    'بداية الفترة (بتوقيت سوريا)': 'Window start (Syria time)',
    'نهاية الفترة (بتوقيت سوريا)': 'Window end (Syria time)',
    'مهلة الإيقاظ (دقائق قبل موعد الإرسال)': 'Wake lead time (minutes before the send)',
    ...
    'لن يستيقظ الخادم أبداً: لا تذكيرات ولا لوحات يومية إطلاقاً حتى تعيد تفعيل أحد المفتاحين (الإعدادات تبقى محفوظة). متابعة؟': 'The server will never wake: no reminders and no daily boards at all until you re-enable one of the two switches (settings are kept). Continue?',
    'النبضة متوقفة و«إجبار الإيقاظ» مفعّل: الخادم سينام دائماً ويستيقظ فقط قبيل كل موعد إرسال — التذكيرات واللوحات ستصل في أوقاتها. متابعة؟': 'Tick off with “force wake” on: the server will always sleep and wake only shortly before each send — reminders and boards will arrive on time. Continue?',
    '«إجبار الإيقاظ» معطّل: أي تذكير أو لوحة موعدها خارج الفترات المحددة لن يصل. متابعة؟': '“Force wake” is off: any reminder or board scheduled outside the set windows will NOT be delivered. Continue?',
    'لم تحدد أي يوم عمل — سيقتصر العمل على «إجبار الإيقاظ» قبيل مواعيد الإرسال فقط. متابعة؟': 'No working day selected — only “force wake” will run shortly before scheduled sends. Continue?',
